...
# Run the test
echo "Running command: /D/Files/Session/INT603/test603/jmeter30/apache-jmeter.3.0/bin/jmeter -t ${TESTSCRIPT} -n -l ${TESTRESULTS} -JWINDIR=${WINDIR} -JSCENARIO=${SCENARIO}"
/D/Files/Session/INT603/test603/jmeter30/apache-jmeter-3.0/bin/jmeter -t ${TESTSCRIPT} -n -l ${TESTRESULTS} -JWINDIR=${WINDIR} -JSCENARIO=${SCENARIO}
